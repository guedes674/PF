...
isPrime :: Int -> Bool
isPrime 1 = False
isPrime x | aux3 x 2 == 0 = True
          | otherwise = False

aux3 :: Int -> Int -> Int
aux3 x acc | acc >= x = 0
           | mod x acc == 0 = 1
           | otherwise = aux3 x (acc+1)
...
union' [] l = []
union' l [] = l
union' (x:xs) (y:ys) | aux5 x (y:ys) = union' (x:xs) ys
...
insereMSet _ [] = []
insereMSet x ((a,b):t) | elemMSet x ((a,b):t) == False = ((a,b):t) ++ [(x,1)]
                       | x == a = (a,b+1):t
